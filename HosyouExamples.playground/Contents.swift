...
extension UITextField {
    public func valueChanged() -> Promise<String> {
        ...
        return Promise({ (resolve, reject) in
            self.delegate = Delegate(resolve: resolve)
        })
    }
}

let textField = UITextField(frame: CGRect(x: 0.0, y: 0.0, width: 200, height: 44))
textField.backgroundColor = .red
textField.valueChanged().then { (value) in
    print("\(value)")
}
